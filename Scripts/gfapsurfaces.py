...
def detect_gfap_surfaces(channel_index, region_name, smooth_width=0.9, contrast_width=100.0, threshold=3800.0):
    print(f"Detecting GFAP surfaces for {region_name} (Channel {channel_index})...")
    try:
        ...
        surfaces.SetName(f"{region_name} GFAP")
        surfaces.SetVisible(True)
        imaris_app.GetSurpassScene().AddChild(surfaces, -1)
        print(f"{region_name} GFAP surfaces added to scene! ({surfaces.GetNumberOfSurfaces()} surfaces)")
    except Exception as e:
        print(f"Error detecting surfaces for {region_name}: {e}")

# Detect in Hippocampus
detect_gfap_surfaces(channel_index=10, region_name="Hippocampus")

# Detect in Cortex
detect_gfap_surfaces(channel_index=11, region_name="Cortex")
